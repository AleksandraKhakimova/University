...
def func(x, t):
    """Функция правой части уравнения (неоднородность) - в данном случае 0"""
    return 0
...
def explicit(K, t, tau, h, x, approx_st, approx_bo):
    ...
    for j in range(N):
        ...
        # Аппроксимация начальной скорости u_t(x,0) = 2cos(x)
        if approx_st == 1:
            # Первый порядок: u_t(x,0) ≈ (U[1,j] - U[0,j])/τ = 2cos(x)
            U[1][j] = 0 + 2 * np.cos(x[j]) * tau
        elif approx_st == 2:
            # Второй порядок с использованием уравнения
            # Из уравнения: u_tt(x,0) = u_xx(x,0) - 3u(x,0)
            u_xx = -2 * np.cos(x[j])  # вторая производная от 2cos(x) для u_t
            u_tt = u_xx - 3 * 0  # так как u(x,0)=0
            U[1][j] = 0 + 2 * np.cos(x[j]) * tau + 0.5 * u_tt * tau**2

    # Основной цикл по времени
    for k in range(1, K - 1):
        t += tau
        for j in range(1, N - 1):
            # Явная схема для уравнения: u_tt = u_xx - 3u
        ...
        if approx_bo == 1:
            ...
            U[k + 1, 0] = func_border1(t)  # u(0,t) = sin(2t)
            U[k + 1, N - 1] = func_border2(t)  # u(π,t) = -sin(2t)
...
def implicit(K, t, tau, h, x, approx_st, approx_bo):
    ...
    for k in range(1, K - 1):
        ...
        for j in range(1, N - 1):
            a[j] = tau**2 / h**2                    # Коэффициент при U[j-1]
            b[j] = -2 * tau**2 / h**2 - 3 * tau**2 - 1  # Коэффициент при U[j]
            c[j] = tau**2 / h**2                    # Коэффициент при U[j+1]
            d[j] = -2 * U[k, j] + U[k - 1, j]
